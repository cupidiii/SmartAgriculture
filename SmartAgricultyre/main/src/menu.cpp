...
static Menu_Direction BtnScan(void)
{
    ...
    const int buttonPin1 = D5; // 定义按键引脚
    const int buttonPin2 = D6;
    const int buttonPin3 = D7;

    pinMode(buttonPin1, INPUT_PULLUP);//设置上拉输入
    pinMode(buttonPin2, INPUT_PULLUP);
    pinMode(buttonPin3, INPUT_PULLUP);

    int reading1 = digitalRead(buttonPin1); // 获取当前按键状态
...
static void Process_App_Run(xpMenu Menu, xpItem item, Menu_Direction Direction)
{
    ...
    case LOOP_FUNCTION:
        ...
        break;
    case WAVE:
        if (item->itemFunction == NULL)plotCure(Menu);
        else (item->itemFunction)(Menu); // 执行项目的函数
        if(Menu->dir == MENU_ENTER)Change_MenuState(Menu, APP_QUIT); // 如果项目状态为进入菜单，则改变菜单状态为函数退出
        break;
    case _TEXT_:
